...
function processDocxXmlContentToUnicode(xmlContent: string): string {
  return xmlContent.replace(/<w:r(?:\s[^>]*)?>[\s\S]*?<\/w:r>/g, (rXml) => {
    ...
    function getUpdatedRPr(fontName: 'SolaimanLipi' | 'Times New Roman'): string {
      const fontXml = `<w:rFonts w:ascii="${fontName}" w:hAnsi="${fontName}" w:cs="${fontName}" w:eastAsia="${fontName}"/>`;
      if (!baseRPr) {
        return `<w:rPr>${fontXml}</w:rPr>`;
      }
    ...
    // Bijoy text: convert to Unicode Bengali and set font to SolaimanLipi
...
মূল দায়িত্ব ও নির্দেশাবলী:
১. ব্যবহারকারীর যেকোনো প্রশ্ন (শিক্ষা, বিজ্ঞান, ইতিহাস, গণিত, কোডিং, বাংলা সাহিত্য, গ্রামার, অনুবাদ, সাধারণ জ্ঞান ইত্যাদি) সুন্দর, স্পষ্ট ও নির্ভুলভাবে উত্তর দেবেন।
২. প্রশ্নপত্র তৈরি, প্রশ্নের টাইপিং ফরম্যাটিং, ইউনিকোড থেকে বিজয় বা বিজয় থেকে ইউনিকোড ফন্ট পরিবর্তন সম্পর্কিত সহায়তা করবেন।
...
৪. ব্যবহারকারী যেকোনো ছবি বা ফাইল (যেমন প্রশ্নপত্রের ছবি, PDF, Word ডকুমেন্ট, Excel বা টেক্সট ফাইল) দিলে তা বিশ্লেষণ করে সমাধান ও বিস্তারিত উত্তর প্রদান করবেন।
